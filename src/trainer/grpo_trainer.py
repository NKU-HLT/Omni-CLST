...
class GRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            # elif "Qwen25-Omni" or "Ke-Omni-R" in model_id:
            else:
                model = Qwen2_5OmniThinkerForConditionalGeneration.from_pretrained(model, low_cpu_mem_usage=True, attn_implementation="sdpa", torch_dtype=torch.float16, **model_init_kwargs)
            # else:
        ...
        if args.beta > 0:
            if is_deepspeed_zero3_enabled():
                ...
                # elif "Qwen25-Omni" or "Ke-Omni-R" in model_id:
                    ...
                    self.ref_model = Qwen2_5OmniThinkerForConditionalGeneration.from_pretrained(model_id, attn_implementation=None, **model_init_kwargs)
                # else:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        #["<|im_start|>system\nYou are a helpful assistant.<|im_end|>\n<|im_start|>user\n<|audio_bos|><|AUDIO|><|audio_eos|>What happened in the audio? Please choose the answer from the following options: ['The sheep are fighting with people', 'The dog is playing with me', 'Fight with the calf', 'Playing with cats']. Output the thinking process (less than 50 words) in <think> </think> and final answer in <answer> </answer>.<|im_end|>\n<|im_start|>assistant\n"]
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                output_reward_func = reward_func(prompts=prompts, completions=completions, is_think=self.think, **reward_kwargs)
                rewards_per_func[:, i] = torch.tensor(output_reward_func, dtype=torch.float32, device=device) * self.reward_weights[i].to(device)
